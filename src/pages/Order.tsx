...
function creditStatus(score: number, entryPct: number): 'APROVADO' | 'REPROVADO' | 'EM_ANALISE' {
  if ((entryPct >= 0.5 && score < 700) || score > 700) return 'APROVADO'
  if (score >= 501) return 'EM_ANALISE'
  return 'REPROVADO'
}
...
const Order = () => {
  ...
  const validOptionals = (configuration.optionals as unknown as string[]).filter(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentMethod === 'financiamento') {
      try {
        ...
        orderStatus = creditStatus(data.score, entryValue / totalPrice)
      } catch (err) {
    ...
    const finalPrice = paymentMethod === 'financiamento' ? entryValue + totalFinanced : totalPrice

    const { order, error } = await createOrder({
      configuration: { ...configuration, optionals: validOptionals },
              ...
              <section className="bg-card rounded-lg p-6 shadow-elegant">
                ...
                <div className="flex gap-4">
                  ...
                  >
                    ...
                    <p className="text-xl font-display font-semibold mt-1">{formatPrice(totalPrice)}</p>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <p className="text-xl font-display font-semibold mt-1">12x de {formatPrice(installmentValue)}</p>
                  </button>
                </div>
                  ...
                  <div className="mt-4 space-y-4 animate-fade-in">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className="font-mono"
                      />
                      <p className="text-xs text-muted-foreground">Máximo: {formatPrice(totalPrice)}</p>
                    </div>
                    <div className="p-4 bg-secondary rounded-lg">
                      <p className="text-sm text-muted-foreground">
                        Valor a financiar: <span className="font-medium">{formatPrice(amountToFinance)}</span>
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Parcela (12x): <span className="font-medium">{formatPrice(installmentValue)}</span>
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Taxa de juros: <span className="font-medium">2% a.m.</span>
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Total financiado: <span className="font-medium">{formatPrice(totalFinanced)}</span>
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Juros totais:{' '}
          ...
          <div className="lg:col-span-1">
            <div className="bg-card rounded-lg p-6 shadow-elegant sticky top-8">
              ...
              <div className="aspect-video bg-stage rounded-lg overflow-hidden mb-4 flex items-center justify-center">
                <img
                  src={CAR_IMAGES[configuration.exteriorColor][configuration.wheelType]}
                  alt="Velô Sprint"
              ...
              <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
                <li className="flex justify-between">
                  <span>Cor</span>
                  <span className="text-foreground">{COLOR_LABELS[configuration.exteriorColor]}</span>
                </li>
                <li className="flex justify-between">
                  <span>Interior</span>
                ...
                {(validOptionals as unknown as string[]).map((opt) => (
              ...
              <div className="border-t border-border mt-4 pt-4">
                <div className="flex justify-between items-center">
                  ...
                  <span data-testid="summary-total-price" className="text-2xl font-display font-semibold">
